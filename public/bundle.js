...
var _friendAPIManager = _interopRequireDefault(require("./friendAPIManager"));
...
const buildFriends = userId => {
  ...
  _friendAPIManager.default.getAllFriendsByFriend(userId).then(friends => {
    friends.forEach(friend => {
      htmlString += `<h4>${friend.user.username}<h4>`;
    });
  }).then(() => {
    _friendAPIManager.default.getAllFriendsByUser(userId).then(friends => {
      friends.forEach(friend => {
        const otherFriendId = friend.otherFriendId;

        _friendAPIManager.default.getSingleFriend(otherFriendId).then(singleFriend => {
...
},{"./friendAPIManager":1}],4:[function(require,module,exports){
"use strict";
